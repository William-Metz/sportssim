...
|----|------|--------|----------|-------|
| 054 | **Pre-Opening Day Final Check** | ⏳ QUEUED | P0 | End-to-end test on March 26: MLB lineup pipeline, F5 unders scan, Opening Day Playbook, weather integration, auto-scanner MLB flow. All MUST work for March 27. **4 DAYS LEFT.** |
| 057 | **NBA Seeding Sim → Futures Value Bridge** | ⏳ NEW | P1 | Seeding sim is LIVE (v54.0) but not wired to futures odds. Key battles: TOR/ATL (E 6/7, deadlocked), MIN/DEN (W 4/5, 0.3 gap), PHI/TOR (E 5/6). Seeding probabilities should feed into championship/series futures value detection. |
| 058 | **Opening Day Weather Pre-Check** | ⏳ NEW | P1 | Pull 5-day forecasts for all March 27 venues NOW. Pre-cache cold/wind data for outdoor stadiums. NYY@SF (Oracle Park wind), OAK@TOR (dome=OK). Critical for F5 unders and total accuracy. |
| 059 | **Spring Training Signal Validation** | ⏳ NEW | P2 | LAD (19-8) and SF (19-9) dominating Cactus League. Cross-check preseason-tuning.js weights — are ST leaders getting appropriate signal boost? Low-impact but free edge if calibrated. |
| 060 | **NFL Win Total Futures Model** | ⏳ NEW | P2 | NFL Draft April 24 (33 days). Win total futures are live on books NOW. Build NFL base model + roster change tracker before draft moves lines. 4-week early mover window. |
...
*✅ Phase 2.75 COMPLETE + Phase 3 PARTIAL: NHL playoffs, goalie starters, NBA seeding sim all LIVE*
*✅ CRITICAL FIXES: Scanner value detection, watchdog, field name mismatch — ALL FIXED*
*🔧 P0: Pre-Opening Day final check (task 054) — must pass by March 26*
*🔧 P1: Seeding→futures bridge (057), OD weather pre-check (058)*
*🔧 P2: ST validation (059), NFL win totals (060)*
*Next build priorities: 058 (weather) → 057 (seeding→futures) → 054 (OD check on March 26) → 060 (NFL)*
