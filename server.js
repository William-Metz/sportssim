...
// Global OOM/crash protection — keep server alive
...
try { liveExecution = require('./services/od-live-execution'); } catch (e) { console.error('[server] Live Execution Engine not loaded:', e.message); }
let odPreflightV120 = null;
...
let odQuickCard = null;
try { odQuickCard = require('./services/od-quick-card'); } catch (e) { console.error('[server] OD Quick Card not loaded:', e.message); }
let odOpenerAnalysis = null;
try { odOpenerAnalysis = require('./services/od-opener-analysis'); } catch (e) { console.error('[server] OD Opener Analysis not loaded:', e.message); }
let mlbStarterSync = null;
try { mlbStarterSync = require('./services/mlb-starter-sync'); } catch (e) { console.error('[server] MLB Starter Sync not loaded:', e.message); }
let openerEveVerification = null;
try { openerEveVerification = require('./services/opener-eve-verification'); } catch (e) { console.error('[server] Opener Eve Verification not loaded:', e.message); }
let pitcherBayesian = null;
try { pitcherBayesian = require('./services/pitcher-bayesian-update'); } catch (e) { console.error('[server] Pitcher Bayesian Update not loaded:', e.message); }
let odMorningProtocol = null;
try { odMorningProtocol = require('./services/od-morning-protocol'); } catch (e) { console.error('[server] OD Morning Protocol not loaded:', e.message); }
...
// autoGradePipeline already declared above (line 137)
let odLiveOdds = null;
...
app.get('/api/health/deep', (req, res) => {
  ...
  const services = {
    ...
    openerEveVerification: !!openerEveVerification,
    pitcherBayesian: !!pitcherBayesian,
  };
...
// ==================== LIVE BET EXECUTION ENGINE ENDPOINTS ====================

// Comprehensive T-1 pre-flight check — validates ALL systems
app.get('/api/od/preflight-v120', async (req, res) => {
  try {
    if (!odPreflightV120) return res.status(503).json({ error: 'Preflight v120 not loaded' });
    const result = await odPreflightV120.runPreFlightCheck();
    res.json(result);
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
// ==================== OD QUICK CARD (FAST! <3s) ====================
app.get('/api/od/quick-card', async (req, res) => {
  try {
    if (!odQuickCard) return res.status(503).json({ error: 'Quick Card not loaded' });
    const day = req.query.day || 'all';
    const bankroll = parseInt(req.query.bankroll || '1000');
    const minEdge = parseFloat(req.query.minEdge || '2.0');
    const card = await odQuickCard.generateQuickCard({ day, bankroll, minEdge });
    res.json(card);
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
app.get('/api/od/quick-card/day/:day', async (req, res) => {
  try {
    if (!odQuickCard) return res.status(503).json({ error: 'Quick Card not loaded' });
    const card = await odQuickCard.generateQuickCard({ day: parseInt(req.params.day) });
    res.json(card);
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
app.get('/api/od/quick-card/smash', async (req, res) => {
  try {
    if (!odQuickCard) return res.status(503).json({ error: 'Quick Card not loaded' });
    const card = await odQuickCard.generateQuickCard({ day: 'all', minEdge: 5.0 });
    const smash = (card.tiers?.smash || []).concat(card.tiers?.strong || []);
    res.json({
      generated: card.generated,
      oddsSource: card.oddsSource,
      countdown: card.countdown,
      smashPlays: smash,
      totalEV: smash.reduce((sum, p) => sum + ((p.wager || 0) * (p.edge || 0) / 100), 0),
    });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
app.get('/api/od/quick-card/game/:away/:home', (req, res) => {
  try {
    if (!odQuickCard) return res.status(503).json({ error: 'Quick Card not loaded' });
    const view = odQuickCard.gameQuickView(req.params.away, req.params.home);
    res.json(view);
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
// ==================== OD MORNING PROTOCOL ====================
app.get('/api/od/morning-protocol', async (req, res) => {
  try {
    if (!odMorningProtocol) return res.status(503).json({ error: 'Morning Protocol not loaded' });
    const result = await odMorningProtocol.runMorningProtocol();
    res.json(result);
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
app.get('/api/od/morning-protocol/weather', async (req, res) => {
  try {
    if (!odMorningProtocol) return res.status(503).json({ error: 'Morning Protocol not loaded' });
    const odQC = odQuickCard || require('./services/od-quick-card');
    const weather = await odMorningProtocol.checkWeather(odQC.OD_DAY1);
    res.json({ day: 1, venues: weather });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

app.get('/api/od/morning-protocol/health', (req, res) => {
  try {
    if (!odMorningProtocol) return res.status(503).json({ error: 'Morning Protocol not loaded' });
    res.json(odMorningProtocol.checkSystemHealth());
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
...
app.get('/api/od/live-execution', async (req, res) => {
  try {
    ...
    const day = parseInt(req.query.day || '1');
    const plan = await liveExecution.generateExecutionPlan(day);
    res.json(plan);
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
// Quick cached status
app.get('/api/od/live-execution/quick', (req, res) => {
  try {
    if (!liveExecution) return res.status(503).json({ error: 'Live Execution Engine not loaded' });
    res.json(liveExecution.getQuickStatus());
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

// Single game deep dive with all sub-models
app.get('/api/od/live-execution/game/:away/:home', async (req, res) => {
  try {
    if (!liveExecution) return res.status(503).json({ error: 'Live Execution Engine not loaded' });
    const { away, home } = req.params;
    const day = parseInt(req.query.day || '1');
    const result = await liveExecution.gameDeepDive(away, home, day);
    res.json(result);
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
app.post('/api/od/live-execution/refresh', async (req, res) => {
  try {
    if (!liveExecution) return res.status(503).json({ error: 'Live Execution Engine not loaded' });
    const day = parseInt(req.query.day || '1');
    const plan = await liveExecution.generateExecutionPlan(day);
...
// Edge movement tracker — are our edges growing or shrinking?
app.get('/api/od/live-execution/edge-movement', (req, res) => {
  try {
    if (!liveExecution) return res.status(503).json({ error: 'Live Execution Engine not loaded' });
    res.json(liveExecution.getEdgeMovement());
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

// ==================== NYY@SF SEASON OPENER ANALYSIS ====================

// Full opener analysis — THE FIRST GAME OF THE SEASON
app.get('/api/od/opener', async (req, res) => {
  try {
    if (!odOpenerAnalysis) return res.status(503).json({ error: 'Opener Analysis not loaded' });
    const force = req.query.refresh === '1';
    const analysis = await odOpenerAnalysis.getAnalysis(force);
    res.json(analysis);
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
// Quick opener summary (cached, fast)
app.get('/api/od/opener/quick', (req, res) => {
  try {
    if (!odOpenerAnalysis) return res.status(503).json({ error: 'Opener Analysis not loaded' });
    res.json(odOpenerAnalysis.getQuickSummary());
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

// K props for Cole + Webb
app.get('/api/od/opener/k-props', (req, res) => {
  try {
    if (!odOpenerAnalysis) return res.status(503).json({ error: 'Opener Analysis not loaded' });
    res.json(odOpenerAnalysis.analyzeKProps());
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

// Betting card — actionable plays
app.get('/api/od/opener/betting-card', async (req, res) => {
  try {
    if (!odOpenerAnalysis) return res.status(503).json({ error: 'Opener Analysis not loaded' });
    const analysis = await odOpenerAnalysis.getAnalysis();
    res.json(analysis.bettingCard || { error: 'No betting card generated' });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

// NRFI analysis for the opener
app.get('/api/od/opener/nrfi', (req, res) => {
  try {
    if (!odOpenerAnalysis) return res.status(503).json({ error: 'Opener Analysis not loaded' });
    res.json(odOpenerAnalysis.analyzeNRFI());
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

// ==================== OPENER EVE VERIFICATION — T-24 Hour Comprehensive Check ====================

// Full verification suite for the opener (weather + starters + odds + model + execution plan)
app.get('/api/opener-eve/verify', async (req, res) => {
  try {
    if (!openerEveVerification) return res.status(503).json({ error: 'Opener Eve Verification not loaded' });
    const result = await openerEveVerification.verifyOpener();
    res.json(result);
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
// Detailed weather for Oracle Park
app.get('/api/opener-eve/weather', async (req, res) => {
  try {
    if (!openerEveVerification) return res.status(503).json({ error: 'Opener Eve Verification not loaded' });
    const weather = await openerEveVerification.fetchWeather(
      openerEveVerification.ORACLE_PARK.lat,
      openerEveVerification.ORACLE_PARK.lon,
      '2026-03-25'
    );
    const windImpact = openerEveVerification.getOracleWindImpact(
      weather.hourly?.wind_direction_10m?.[20] || 270,
      weather.hourly?.wind_speed_10m?.[20] || 10
    );
    const tempImpact = openerEveVerification.getTempImpact(
      weather.hourly?.temperature_2m?.[20] || 55
    );
    res.json({
      park: openerEveVerification.ORACLE_PARK,
      raw: weather,
      windImpact,
      tempImpact,
      hoursToFirstPitch: openerEveVerification.getHoursToFirstPitch()
    });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
// Live odds from all books for the opener
app.get('/api/opener-eve/odds', async (req, res) => {
  try {
    if (!openerEveVerification) return res.status(503).json({ error: 'Opener Eve Verification not loaded' });
    // Pull from the full verification
    const result = await openerEveVerification.verifyOpener();
    res.json({
      odds: result.liveOdds || null,
      checks: result.checks?.odds || null,
      model: result.modelPrediction || null,
...
// Final execution plan with timing
app.get('/api/opener-eve/execution', async (req, res) => {
  try {
    if (!openerEveVerification) return res.status(503).json({ error: 'Opener Eve Verification not loaded' });
    const result = await openerEveVerification.verifyOpener();
    res.json({
...
// Game day action timeline
app.get('/api/opener-eve/timeline', (req, res) => {
  try {
    if (!openerEveVerification) return res.status(503).json({ error: 'Opener Eve Verification not loaded' });
    res.json({
      timeline: openerEveVerification.getGameDayTimeline(),
      hoursToFirstPitch: openerEveVerification.getHoursToFirstPitch()
    });
  } catch (e) {
    res.status(500).json({ error: e.message });
...
// Day 1 (March 26) full slate verification
app.get('/api/od/d1-eve', async (req, res) => {
  try {
    if (!openerEveVerification) return res.status(503).json({ error: 'Opener Eve Verification not loaded' });
    const result = await openerEveVerification.verifyDay1Slate();
    res.json(result);
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
// ==================== MLB STARTER SYNC — Auto-verify starters from MLB Stats API ====================

// Run starter sync — compare our OD starters to MLB Stats API probablePitchers
app.get('/api/mlb/starter-sync', async (req, res) => {
  try {
    if (!mlbStarterSync) return res.status(503).json({ error: 'MLB Starter Sync not loaded' });
    const result = await mlbStarterSync.syncStarters();
    res.json(result);
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
// Cached sync status
app.get('/api/mlb/starter-sync/status', (req, res) => {
  try {
    if (!mlbStarterSync) return res.status(503).json({ error: 'MLB Starter Sync not loaded' });
    const cached = mlbStarterSync.getCachedStatus();
    if (cached) return res.json(cached);
    res.json({ status: 'No sync data yet — hit /api/mlb/starter-sync first' });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

// ==================== PITCHER BAYESIAN UPDATE ENDPOINTS (v127.0) ====================

// Full sync — fetch game logs for all pitchers, update Bayesian blend
app.post('/api/mlb/pitcher-update/sync', async (req, res) => {
  try {
    if (!pitcherBayesian) return res.status(503).json({ error: 'Pitcher Bayesian Update not loaded' });
    const result = await pitcherBayesian.fullSync();
...
// Model health — how much live pitcher data do we have?
app.get('/api/mlb/pitcher-update/health', (req, res) => {
  try {
    if (!pitcherBayesian) return res.status(503).json({ error: 'Pitcher Bayesian Update not loaded' });
    res.json(pitcherBayesian.getModelHealth());
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

// Get blended stats for a specific pitcher
app.get('/api/mlb/pitcher-update/:pitcher', (req, res) => {
  try {
    if (!pitcherBayesian) return res.status(503).json({ error: 'Pitcher Bayesian Update not loaded' });
    const name = decodeURIComponent(req.params.pitcher);
    const update = pitcherBayesian.getPitcherUpdate(name);
    if (!update) return res.status(404).json({ error: `No data for ${name}` });
    res.json(update);
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
// Get team's pitcher updates
app.get('/api/mlb/pitcher-update/team/:team', (req, res) => {
  try {
    if (!pitcherBayesian) return res.status(503).json({ error: 'Pitcher Bayesian Update not loaded' });
    const team = req.params.team.toUpperCase();
    res.json(pitcherBayesian.getTeamPitchers(team));
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

// Regression candidates — the money plays
app.get('/api/mlb/pitcher-update/regression/:direction', (req, res) => {
  try {
    if (!pitcherBayesian) return res.status(503).json({ error: 'Pitcher Bayesian Update not loaded' });
    const direction = req.params.direction; // lucky, unlucky, overperforming, underperforming, all
    const minIP = parseInt(req.query.minIP) || 15;
    const candidates = pitcherBayesian.getRegressionCandidates(direction, minIP);
    res.json({ direction, minIP, count: candidates.length, candidates });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

// Top deviations — biggest gaps between actual and projected
app.get('/api/mlb/pitcher-update/deviations', (req, res) => {
  try {
    if (!pitcherBayesian) return res.status(503).json({ error: 'Pitcher Bayesian Update not loaded' });
    const limit = parseInt(req.query.limit) || 20;
    res.json(pitcherBayesian.getTopDeviations(limit));
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

// Leaderboard — top/bottom pitchers by blended stat
app.get('/api/mlb/pitcher-update/leaderboard/:stat', (req, res) => {
  try {
    if (!pitcherBayesian) return res.status(503).json({ error: 'Pitcher Bayesian Update not loaded' });
    const stat = req.params.stat; // era, fip, k9, bb9, whip
    const limit = parseInt(req.query.limit) || 30;
    const direction = req.query.dir || (stat === 'k9' ? 'desc' : 'asc');
    res.json(pitcherBayesian.getLeaderboard(stat, limit, direction));
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
...
// Save daily snapshot
app.post('/api/mlb/pitcher-update/snapshot', (req, res) => {
  try {
    if (!pitcherBayesian) return res.status(503).json({ error: 'Pitcher Bayesian Update not loaded' });
    res.json(pitcherBayesian.saveDailySnapshot());
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

// ==================== PRE-OD FINAL VALIDATION ENDPOINTS ====================

// Full comprehensive validation — THE LAST GATE
app.get('/api/od/final-validation', async (req, res) => {
  try {
    if (!preOdValidation) return res.status(503).json({ error: 'Pre-OD Validation not loaded' });
    const result = await preOdValidation.runFullValidation();
    res.json(result);
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
// Quick validation — instant health check
app.get('/api/od/final-validation/quick', (req, res) => {
  try {
    if (!preOdValidation) return res.status(503).json({ error: 'Pre-OD Validation not loaded' });
    res.json(preOdValidation.runQuickValidation());
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
